...
RACE_LABELS = {
    'W':'White', 'B':'Black', 'A':'Asian heritage', 'N':'Native American', 'H':'Hispanic', 'O':'Other'
}
...
STATE_NAME_BY_ABBR = {STATE_ABBR_BY_FIPS[f]: name for f, name in state_name_by_fips.items()}
...
for r in read_csv(POP_2019_PATH):
    if r.get('SUMLEV') == '010':
        for year in range(2015, 2020):
            pop_rows.append({'year': year, 'state_abbr': 'US', 'state_fips': '00', 'state_name': 'United States', 'population': int(r[f'POPESTIMATE{year}']), 'population_source_vintage':'Census PEP Vintage 2019', 'population_source_url':'https://www2.census.gov/programs-surveys/popest/datasets/2010-2019/national/totals/nst-est2019-alldata.csv'})
for r in read_csv(POP_2025_PATH):
    if r.get('SUMLEV') == '010':
        for year in range(2020, 2026):
            pop_rows.append({'year': year, 'state_abbr': 'US', 'state_fips': '00', 'state_name': 'United States', 'population': int(r[f'POPESTIMATE{year}']), 'population_source_vintage':'Census PEP Vintage 2025', 'population_source_url':'https://www2.census.gov/programs-surveys/popest/datasets/2020-2025/state/totals/NST-EST2025-ALLDATA.csv'})

pop_by_state_year = {(r['state_abbr'], int(r['year'])): int(r['population']) for r in pop_rows if r['population'] != ''}
...
for r in agencies_raw:
    ...
    out = {
        ...
        'agency_total_shootings_original': int(r['total_shootings']) if r.get('total_shootings','').isdigit() else ''
    }
...
for r in incidents_raw:
    ...
    armed_codes = [c.strip() for c in (r.get('armed_with') or '').split(';') if c.strip()]
    agency_ids = [a.strip() for a in (r.get('agency_ids') or '').split(';') if a.strip()]
    out = {
        ...
        'armed_unknown_or_undetermined': any(c in ('unknown','undetermined') for c in armed_codes),
...
for r in clean_incidents:
    ...
    if r['is_unarmed']: state_year[key]['unarmed_count'] += 1
    if r['armed_with_gun']: state_year[key]['armed_with_gun_count'] += 1
    if r['was_mental_illness_related'] is True: state_year[key]['mental_illness_related_count'] += 1
    if r['body_camera'] is True: state_year[key]['body_camera_count'] += 1
    if r['flee_status'] != 'not_fleeing': state_year[key]['fleeing_or_missing_count'] += 1
    if r['race_known']: state_year[key]['race_known_count'] += 1
    if not r['race_known']: state_year[key]['race_missing_count'] += 1
state_year_rows = []
for (year, state), c in sorted(state_year.items()):
    ...
    row = {
        ...
        'population': pop,
        'fatal_shootings_per_100k': round(shootings / denom * 100000, 4) if denom else '',
        'fatal_shootings_per_1m': round(shootings / denom * 1000000, 4) if denom else '',
        'unarmed_count': c['unarmed_count'],
        'unarmed_per_1m': round(c['unarmed_count'] / denom * 1000000, 4) if denom else '',
        'armed_with_gun_count': c['armed_with_gun_count'],
        'armed_with_gun_per_1m': round(c['armed_with_gun_count'] / denom * 1000000, 4) if denom else '',
        'mental_illness_related_count': c['mental_illness_related_count'],
        'mental_illness_related_per_1m': round(c['mental_illness_related_count'] / denom * 1000000, 4) if denom else '',
        'body_camera_count': c['body_camera_count'],
        'body_camera_per_1m': round(c['body_camera_count'] / denom * 1000000, 4) if denom else '',
        'race_known_count': c['race_known_count'],
        'race_missing_count': c['race_missing_count'],
        'population_source_vintage': next((p['population_source_vintage'] for p in pop_rows if p['state_abbr']==state and p['year']==year), ''),
        'population_source_url': next((p['population_source_url'] for p in pop_rows if p['state_abbr']==state and p['year']==year), '')
    }
...
for r in clean_incidents:
    ...
    if r['is_unarmed']: national[y]['unarmed_count'] += 1
    if r['armed_with_gun']: national[y]['armed_with_gun_count'] += 1
    if r['was_mental_illness_related'] is True: national[y]['mental_illness_related_count'] += 1
    if r['body_camera'] is True: national[y]['body_camera_count'] += 1
    if r['race_known']: national[y]['race_known_count'] += 1
    else: national[y]['race_missing_count'] += 1
national_rows = []
for year, c in sorted(national.items()):
    ...
    national_rows.append({
        ...
        'race_known_count': c['race_known_count'],
        'race_missing_count': c['race_missing_count'],
        'population_source_vintage': 'Census PEP Vintage 2019' if year <= 2019 else 'Census PEP Vintage 2025'
    })
...
for r in clean_incidents:
    ...
    if r['is_unarmed']: state_overall[state]['unarmed_count'] += 1
    if r['was_mental_illness_related'] is True: state_overall[state]['mental_illness_related_count'] += 1
state_overall_rows = []
for state, c in sorted(state_overall.items()):
    avg_pop = round(sum(pop_avg[state]) / len(pop_avg[state])) if pop_avg[state] else ''
    years = 10
    annualized_rate = c['fatal_shootings'] / avg_pop / years * 1_000_000 if avg_pop else ''
    state_overall_rows.append({
        ...
        'annualized_fatal_shootings_per_1m': round(annualized_rate, 4) if annualized_rate != '' else '',
        'unarmed_count_2015_2024': c['unarmed_count'],
        'mental_illness_related_count_2015_2024': c['mental_illness_related_count']
    })
state_overall_rows.sort(key=lambda x: x['annualized_fatal_shootings_per_1m'] if x['annualized_fatal_shootings_per_1m'] != '' else -1, reverse=True)
rank=1
...
for (year,label), c in sorted(race_year.items()):
    ...
    race_year_rows.append({
        ...
        'rate_per_1m_total_us_population_NOT_GROUP_SPECIFIC': round(c['count_any_mention']/pop*1_000_000,4) if pop else '',
...
# Data dictionary.
data_dictionary = [
    {'table':'clean_incidents','field':'incident_id','definition':'Unique Fatal Force incident/victim ID from source v2 file.'},
    {'table':'clean_incidents','field':'year/month/quarter','definition':'Derived from incident_date for time-series analysis.'},
    {'table':'clean_incidents','field':'state_fips/state_name','definition':'Added using Census/USPS state crosswalk to support joins with population denominators.'},
    {'table':'clean_incidents','field':'race_label','definition':'Human-readable expansion of source race/ethnicity codes; multi-value source records are semicolon-expanded in the label.'},
    {'table':'clean_incidents','field':'is_unarmed','definition':'True when armed_with includes unarmed.'},
    {'table':'clean_incidents','field':'armed_with_gun','definition':'True when armed_with includes gun.'},
    {'table':'clean_incidents','field':'agency_count','definition':'Count of semicolon-delimited agencies in agency_ids.'},
    {'table':'incident_agency_bridge','field':'agency_id','definition':'One row per incident-agency pair. Use this bridge for agency joins; do not naïvely join incident.agency_ids to agencies.'},
    {'table':'state_population','field':'population','definition':'Annual Census Population Estimates Program resident population, July 1 estimates unless Census documentation specifies base values.'},
    {'table':'state_year_rates','field':'fatal_shootings_per_1m','definition':'fatal_shootings / state population * 1,000,000.'},
    {'table':'state_overall_rates','field':'annualized_fatal_shootings_per_1m','definition':'2015-2024 fatal shooting count divided by average 2015-2024 state population and 10 years, then multiplied by 1,000,000.'},
    {'table':'race_year_numerator_summary','field':'rate_per_1m_total_us_population_NOT_GROUP_SPECIFIC','definition':'Screening metric only. Uses total U.S. population, not race-specific denominators.'},
]
...
outputs = {
    ...
    'race_year_numerator_summary': os.path.join(OUT_DIR, 'race_year_numerator_summary.csv'),
...
checks = {
    ...
    'missing_population_state_year_rows': sum(1 for r in state_year_rows if r['population'] == ''),
