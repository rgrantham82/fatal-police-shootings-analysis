...
STATE_REGION_DIVISION = {
    ...
    'MO': ('Midwest','West North Central'), 'NE': ('Midwest','West North Central'), 'ND': ('Midwest','West North Central'), 'SD': ('Midwest','West North Central'),
    'DE': ('South','South Atlantic'), 'DC': ('South','South Atlantic'), 'FL': ('South','South Atlantic'), 'GA': ('South','South Atlantic'),
    'MD': ('South','South Atlantic'), 'NC': ('South','South Atlantic'), 'SC': ('South','South Atlantic'), 'VA': ('South','South Atlantic'), 'WV': ('South','South Atlantic'),
    'AL': ('South','East South Central'), 'KY': ('South','East South Central'), 'MS': ('South','East South Central'), 'TN': ('South','East South Central'),
    'AR': ('South','West South Central'), 'LA': ('South','West South Central'), 'OK': ('South','West South Central'), 'TX': ('South','West South Central'),
...
AGENCY_TYPES = ['local_police', 'sheriff', 'state_police', 'federal', 'local_other', 'state_other', 'other']
...
inc_aggs: Dict[Tuple[str, int], Dict[str, Any]] = defaultdict(lambda: defaultdict(float))
...
for key, a in list(inc_aggs.items()):
    ...
    for raw, share in [
        ('unarmed_count_incident', 'unarmed_share'), ('gun_count_incident', 'gun_share'), ('vehicle_count_incident', 'vehicle_share'),
        ('mental_count_incident', 'mental_illness_share'), ('bodycam_count_incident', 'body_camera_share'), ('male_count_incident', 'male_share'),
    ...
    for t in THREAT_KEYS:
        clean = t.replace(' ', '_')
        a[f'threat_{clean}_share'] = safe_div(a.get(f'threat_{clean}_count_incident', 0), n)
    for fl in FLEE_KEYS:
        clean = fl.replace(' ', '_')
...
bridge_aggs: Dict[Tuple[str, int], Dict[str, Any]] = defaultdict(lambda: defaultdict(float))
...
for key, a in list(bridge_aggs.items()):
    ...
    for typ in AGENCY_TYPES:
        a[f'agency_type_{typ}_share'] = safe_div(a.get(f'agency_type_{typ}_pairs', 0), pairs)
...
for abbr in states:
    ...
    for year in all_years:
        ...
        row = {
            ...
            'population_growth_1yr': safe_div(pop - prior_pop, prior_pop) if prior_pop else 0.0,
        ...
        for name in [
            'unarmed_share','gun_share','vehicle_share','mental_illness_share','body_camera_share','male_share','median_age',
            'multi_agency_share','avg_agency_count',
        ...
        for name in ['unique_agencies', *[f'agency_type_{typ}_share' for typ in AGENCY_TYPES]]:
...
composition_fields = [
    'unarmed_share','gun_share','vehicle_share','mental_illness_share','body_camera_share','male_share','median_age',
    'multi_agency_share','avg_agency_count','unique_agencies',
    *[f'race_{c}_share' for c in RACE_KEYS],
    *[f'threat_{t.replace(" ", "_")}_share' for t in THREAT_KEYS],
    *[f'flee_{fl.replace(" ", "_")}_share' for fl in FLEE_KEYS],
...
for r in all_rows:
    ...
    r['fatal_shootings_roll3_avg'] = statistics.mean(prior_counts) if prior_counts else 0.0
    r['fatal_shootings_roll3_sd'] = statistics.pstdev(prior_counts) if len(prior_counts) > 1 else 0.0
    r['fatal_rate_per_1m_roll3_avg'] = statistics.mean(prior_rates) if prior_rates else 0.0
    r['fatal_rate_per_1m_roll3_sd'] = statistics.pstdev(prior_rates) if len(prior_rates) > 1 else 0.0
    r['fatal_count_lag1_delta'] = r['fatal_shootings_lag1'] - r['fatal_shootings_lag2']
    r['fatal_rate_lag1_delta'] = r['fatal_rate_per_1m_lag1'] - r['fatal_rate_per_1m_lag2']
...
for r in all_rows:
    ...
    for div in divisions:
        r[f'division_{div.replace(" ", "_").replace("-", "_")}'] = 1 if r['division'] == div else 0

# Previous national rate baseline
nat_rates = {}
for year in years:
    total_count = sum(to_float(row_by_key[(s, year)].get('fatal_shootings'), 0.0) for s in states if (s, year) in row_by_key)
    total_pop = sum(to_float(row_by_key[(s, year)].get('population'), 0.0) for s in states if (s, year) in row_by_key)
...
for r in all_rows:
    ...
    r['prev_national_rate_expected_count'] = prev_rate * r.get('population', 0) / 1_000_000

# Headers for enhanced dataset
base_headers = ['state_abbr','state_fips','state_name','region','division','year','is_forecast_year','fatal_shootings','population','log_population','population_growth_1yr','fatal_shootings_per_1m','year_index','post_2020']
current_headers = composition_fields
lag_headers = [
    'fatal_shootings_lag1','fatal_shootings_lag2','fatal_shootings_lag3',
    'fatal_rate_per_1m_lag1','fatal_rate_per_1m_lag2','fatal_rate_per_1m_lag3',
    'fatal_shootings_roll3_avg','fatal_shootings_roll3_sd','fatal_rate_per_1m_roll3_avg','fatal_rate_per_1m_roll3_sd',
    'fatal_count_lag1_delta','fatal_rate_lag1_delta',
    'prev_national_rate_per_1m','prev_national_rate_expected_count',
]
lag_comp_headers = []
for field in composition_fields:
    lag_comp_headers += [f'{field}_lag1', f'{field}_roll3_avg']
onehot_headers = [f'region_{x.replace(" ", "_")}' for x in regions] + [f'division_{x.replace(" ", "_").replace("-", "_")}' for x in divisions]
headers = base_headers + current_headers + lag_headers + lag_comp_headers + onehot_headers
...
def add_feat(name, role, source, leakage, notes):
    feature_dict.append({'field': name, 'role': role, 'source': source, 'leakage_status': leakage, 'notes': notes})
for h in base_headers:
    add_feat(h, 'identifier/target/exposure' if h in ['fatal_shootings','population','fatal_shootings_per_1m'] else 'identifier/static feature', 'cleaned state-year rates + Census population denominators', 'safe' if h != 'fatal_shootings_per_1m' else 'target-derived', 'Base state-year field')
for h in current_headers:
    add_feat(h, 'descriptive current-year composition', 'incident and agency aggregates', 'do_not_use_for_same_year_prediction', 'Current-year descriptive variable; lagged versions are used for prediction')
for h in lag_headers + lag_comp_headers + onehot_headers:
    source = 'engineered lag/rolling feature' if 'lag' in h or 'roll' in h else 'engineered static category'
    add_feat(h, 'predictor', source, 'safe_lagged_or_static', 'Included in enhanced model feature set unless otherwise excluded')
write_csv(OUT / 'enhanced_feature_dictionary.csv', feature_dict, ['field','role','source','leakage_status','notes'])

# Modeling
model_rows = [r for r in all_rows if r['year'] >= 2018 and r['year'] <= 2024 and r['fatal_shootings'] != '']
...
feature_cols = [
    'population','log_population','population_growth_1yr','year_index','post_2020',
    'fatal_shootings_lag1','fatal_shootings_lag2','fatal_shootings_lag3',
    'fatal_rate_per_1m_lag1','fatal_rate_per_1m_lag2','fatal_rate_per_1m_lag3',
    'fatal_shootings_roll3_avg','fatal_shootings_roll3_sd','fatal_rate_per_1m_roll3_avg','fatal_rate_per_1m_roll3_sd',
    'fatal_count_lag1_delta','fatal_rate_lag1_delta','prev_national_rate_expected_count',
...
X_train = np.array([[to_float(r.get(c), 0.0) for c in usable_features] for r in train_rows], dtype=float)
y_train = np.array([to_float(r['fatal_shootings']) for r in train_rows], dtype=float)
X_test = np.array([[to_float(r.get(c), 0.0) for c in usable_features] for r in test_rows], dtype=float)
y_test = np.array([to_float(r['fatal_shootings']) for r in test_rows], dtype=float)
X_fore = np.array([[to_float(r.get(c), 0.0) for c in usable_features] for r in forecast_rows], dtype=float)

models = {
    'ridge_enhanced_lagged': Pipeline([('scale', StandardScaler()), ('model', RidgeCV(alphas=[0.01,0.1,1,3,10,30,100,300]))]),
    'poisson_enhanced_lagged': Pipeline([('scale', StandardScaler()), ('model', PoissonRegressor(alpha=3.0, max_iter=500))]),
    'random_forest_enhanced_lagged': RandomForestRegressor(n_estimators=200, random_state=42, min_samples_leaf=5, max_features='sqrt', n_jobs=-1),
}
...
# Baselines
predictions_test['lag1_baseline'] = np.array([to_float(r['fatal_shootings_lag1']) for r in test_rows], dtype=float)
predictions_test['roll3_baseline'] = np.array([to_float(r['fatal_shootings_roll3_avg']) for r in test_rows], dtype=float)
predictions_test['prev_national_rate_baseline'] = np.array([to_float(r['prev_national_rate_expected_count']) for r in test_rows], dtype=float)

predictions_fore['lag1_baseline'] = np.array([to_float(r['fatal_shootings_lag1']) for r in forecast_rows], dtype=float)
predictions_fore['roll3_baseline'] = np.array([to_float(r['fatal_shootings_roll3_avg']) for r in forecast_rows], dtype=float)
predictions_fore['prev_national_rate_baseline'] = np.array([to_float(r['prev_national_rate_expected_count']) for r in forecast_rows], dtype=float)
...
ensemble_names = [r['model'] for r in ranked if r['model'] in predictions_test and r['model'] != 'prev_national_rate_baseline'][:4]
if ensemble_names:
    ...
    pred_fore_ens = np.mean(np.vstack([predictions_fore[n] for n in ensemble_names]), axis=0)
    ...
    metrics_rows.append({
        ...
        'rate_mae_per_1m': mean_absolute_error(y_test / pop_test * 1_000_000, pred_ens / pop_test * 1_000_000),
...
write_csv(OUT / 'enhanced_model_evaluation_2023_2024_holdout.csv', metrics_rows, ['model','train_years','test_years','n_train_rows','n_test_rows','count_mae','count_rmse','rate_mae_per_1m','national_total_error','actual_holdout_total','predicted_holdout_total','mean_predicted_count','mean_actual_count'])

# Holdout predictions table
holdout_rows = []
for i, r in enumerate(test_rows):
    base = {
        'state_abbr': r['state_abbr'], 'state_name': r['state_name'], 'region': r['region'], 'division': r['division'], 'year': r['year'],
        'actual_fatal_shootings': to_float(r['fatal_shootings']), 'population': to_int(r['population']),
        'actual_rate_per_1m': to_float(r['fatal_shootings_per_1m']),
        'lag1_count': to_float(r['fatal_shootings_lag1']), 'roll3_avg_count': to_float(r['fatal_shootings_roll3_avg'])
    }
    for name, pred in predictions_test.items():
        base[f'pred_{name}'] = float(pred[i])
        base[f'error_{name}'] = float(pred[i] - to_float(r['fatal_shootings']))
    holdout_rows.append(base)
write_csv(OUT / 'enhanced_state_year_holdout_predictions.csv', holdout_rows, list(holdout_rows[0].keys()))

# 2025 forecast table
forecast_output = []
for i, r in enumerate(forecast_rows):
    row = {
        'state_abbr': r['state_abbr'], 'state_name': r['state_name'], 'region': r['region'], 'division': r['division'], 'year': 2025,
    ...
    for name, pred in predictions_fore.items():
        ...
        row[f'forecast_rate_per_1m_{name}'] = safe_div(float(pred[i]), to_float(r['population'])) * 1_000_000
...
national_forecast['forecast_year_population_total'] = int(sum(to_int(r['population']) for r in forecast_rows))
national_forecast['actual_2024_total'] = int(sum(to_float(row_by_key[(s, 2024)].get('fatal_shootings')) for s in states))
national_forecast['actual_2023_total'] = int(sum(to_float(row_by_key[(s, 2023)].get('fatal_shootings')) for s in states))
with open(OUT / 'enhanced_national_2025_forecast_summary.json', 'w', encoding='utf-8') as f:
...
coefs = ridge.coef_
for feat, coef in sorted(zip(usable_features, coefs), key=lambda x: abs(x[1]), reverse=True)[:40]:
    importance_rows.append({'model': 'ridge_enhanced_lagged', 'feature': feat, 'importance_value': float(coef), 'importance_type': 'standardized_coefficient'})
rf = models['random_forest_enhanced_lagged']
for feat, imp in sorted(zip(usable_features, rf.feature_importances_), key=lambda x: x[1], reverse=True)[:40]:
    importance_rows.append({'model': 'random_forest_enhanced_lagged', 'feature': feat, 'importance_value': float(imp), 'importance_type': 'feature_importance'})
write_csv(OUT / 'enhanced_model_feature_importance.csv', importance_rows, ['model','feature','importance_value','importance_type'])
...
for name, val in sorted(national_forecast.items()):
    if name not in ['ensemble_members', 'forecast_year_population_total', 'actual_2024_total', 'actual_2023_total']:
